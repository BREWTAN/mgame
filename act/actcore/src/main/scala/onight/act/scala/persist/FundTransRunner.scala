...
//    log.error("onBatch:" + "@@,size=" + vs.size)
    ...
    val updateSQL = "UPDATE T_ACT_FUND SET CUR_BAL = CUR_BAL-(?) WHERE FUND_NO = (?) AND CUR_BAL>(?);"
    //3.update状态
    val updateVals = vs.map(x => {
      if (x._1.FROM_FUND_NO.startsWith("S000")) {
        Seq(x._1.AMT, x._1.FROM_FUND_NO, Some(Double.MinValue))
      } else {
        Seq(x._1.AMT, x._1.FROM_FUND_NO, x._1.AMT)
      }
    })
