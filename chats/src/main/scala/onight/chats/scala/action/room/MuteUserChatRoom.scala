...
@NActorProvider
object BlockUserChatRoom extends RoomModules[PBIRoomOp] {
  override def service = BlockUserChatRoomService
}

object BlockUserChatRoomService extends OLog with PBUtils with LService[PBIRoomOp] {

  override def cmd: String = PBRoomCommand.BLK.name();
  ...
  def onPBPacket(pack: FramePacket, pbo: PBIRoomOp, handler: CompleteHandler) = {
    ...
    if (pbo == null) {
      ...
      bmap.put("is_blocked",java.lang.Boolean.valueOf(true))
