...
@NActorProvider
object ListChatRoom extends RoomModules[PBIRoomOp] {
  override def service = ListChatRoomService
}

object ListChatRoomService extends OLog with PBUtils with LService[PBIRoomOp] {

  override def cmd: String = PBRoomCommand.LST.name();

  //  http://localhost:18080/rom/pblst.do?fh=VLSTROM000000J00&bd={}
  def onPBPacket(pack: FramePacket, pbo: PBIRoomOp, handler: CompleteHandler) = {
    ...
    if (pbo == null) {
      ...
      val objs = CassDAOs.roomInfodao.selectByExample(example);

      log.debug("Enter Room:ret={}", objs)
      val it=objs.iterator()
      while(it.hasNext()){
        val roominfo=it.next().asInstanceOf[RoomInfo];
        val pbiri=pbBeanUtil.toPB[PBIRoomInfo](PBIRoomInfo.newBuilder(),roominfo)
        ret.addPbrooms(pbiri);
      }
      if (objs.size() > 0) {
